...
const char* mdns_name = "m5f3102can";

//セッティングONのときの設定
const char* set_ssid = "m5f3102can";
const char* set_password = "98765432";
int set_ip[] ={ 192, 168, 0, 137 };
...
String htmlString( String headstr,  String footstr ){
  ...
  retstr += "<BR><BR>";  
  retstr += "Usage: http://"+ (String)mdns_name + "/set?";
...
void handleRoot() { //ブラウザのUI
  ...
  server.send(200, "text/html", htmlString( "config  CAN ID & Tx cycle time <BR><BR>","" ) );
  //handleGet();
